...
export default function JoinPropertyScreen() {
  ...
  const joinPropertyById = usePropertyStore((state) => state.joinPropertyById);
  ...
  const handleVerify = async () => {
    ...
    try {
      const property = await joinPropertyById(propertyId.trim());
      setIsLoading(false);

      if (property) {
        // Success case - property found and linked
        router.push({
      ...
      Alert.alert(
        "Join Result", 
        err.message || "We couldn't connect you to this property. Please double-check with your owner.",
        [{ text: "Try Again" }]
      );
    }
  };
        ...
        <View style={styles.content}>
          <Animated.View entering={FadeInUp.duration(600)}>
            ...
            <Text style={styles.subtitle}>
              Enter the unique 6-8 character Bridge ID (e.g. RES-XXXX) shared by your owner to access your digital agreement.
            </Text>
          </Animated.View>

          <Animated.View entering={FadeInUp.delay(200).duration(600)} style={styles.inputContainer}>
            <Text style={styles.inputLabel}>Bridge ID</Text>
